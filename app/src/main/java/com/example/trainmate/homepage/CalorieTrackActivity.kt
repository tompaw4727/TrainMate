...
import com.example.trainmate.controller.UserInfoController
...
class CalorieTrackActivity : AppCompatActivity() {
    ...
    private val nutritionInfoController = NutritionInfoController()
    private var auth: FirebaseAuth = FirebaseAuth.getInstance()
    private var mealList: List<NutritionInfo>? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            nutritionInfoController.postNutritionInfo(auth.currentUser!!.uid, "apple")
            mealList = nutritionInfoController.getAllNutritionInfo(auth.currentUser!!.uid)
        }
        ...
        textView.setOnClickListener {
            ...
            val mealNames = mealList?.map { it.name } ?: listOf()

            val adapter = ArrayAdapter(this, android.R.layout.simple_list_item_1, mealNames)
